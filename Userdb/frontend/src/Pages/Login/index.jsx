    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (response.data.status === "ok") {
                ...
                localStorage.setItem('name', response.data.name);  // Assuming the API returns user name
                localStorage.setItem('email', response.data.email);
                localStorage.setItem('gender', response.data.gender);
                localStorage.setItem('role',response.data.role)  // Assuming the API returns user gender
            ...
            <div className="flex items-center justify-center h-screen bg-gray-200">
                ...
                <form onSubmit={handleSubmit} className='bg-white p-8 rounded-lg shadow-lg w-full max-w-md'>
                    ...
                    <div className='mb-4'>
                        ...
                        <input type="password" name="password" className='w-full p-2 border rounded-lg' onChange={(e) => { setPassword(e.target.value) }} />
                    </div>
